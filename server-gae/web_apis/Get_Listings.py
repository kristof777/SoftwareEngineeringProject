...
class GetListing(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        invalid.update(
            is_valid_filter(values["filter"]) if "filter" in values else {})
        if len(invalid) != 0:
            write_error_to_response(self.response, invalid,
                                    missing_invalid_parameter)
            return
        ...
        if not is_valid_xor(values, "listingIdList", "filter"):
            invalid[invalid_xor_condition[
                'error']] = "ListingIdList and filter can't show up together"
            write_error_to_response(self.response, invalid,
                                    missing_invalid_parameter)
            return
        ...
        if "listingIdList" in values and not is_empty(
            values['listingIdList']):
            ...
            for listingId in listingId_list:
                ...
                if listing_object is not None:
                    listing_info_list.append(
                        create_returned_values_dict(listing_object, values))
        ...
        if "filter" in values and not is_empty(values["filter"]):
            ...
            for key in filter:
                if key in ["bedrooms", "bathrooms", "sqft", "price",
                    ...
                    filtered_listings = Listing.query(
                        Listing in filtered_listings,
            ...
            if "userId" in values and not is_empty(values["userId"]):
                ...
                filtered_listings = Listing.query(
                    Listing not in seen_listings).fetch()
...
def create_returned_values_dict(listing_object, values_dict):
    listing_dict = {}
    if "valuesRequired" in values_dict and not is_empty(
        values_dict['valuesRequired']):
            ...
            if key == "bathrooms":
                if "lower" in filter[key]:
                    if not is_valid_float(filter[key]["lower"]):
                        invalid[invalid_filter_bound[
                            'error']] = "Bathroom lower bound invalid"
                if "upper" in filter[key]:
                    if not is_valid_float(filter[key]["upper"]):
                        invalid[invalid_filter_bound[
                            'error']] = "Bathroom upper bound invalid"
